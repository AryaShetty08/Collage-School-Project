...
public class Collage {
    ...
    public static void scale (Picture source, Picture target) {
        ...
        for (int i = 0; i < target.width(); i++) {
            for (int j = 0; j < target.height(); j++) {
                int sourceCol = i * source.width()  / target.width();
                int sourceRow = j * source.height() / target.height();
    ...
    public void makeCollage () {
        ...
        for(int i = 0; i < collageDimension; i++) {
            for(int j = 0; j < collageDimension; j++) {
                for(int m = 0; m < tileDimension; m++) {
                    for(int n = 0; n < tileDimension; n++) {
                        Color color = tile.get(m, n);
                        collagePicture.set(m+(i*tileDimension), n+(j*tileDimension), color);
                    }
                }
            }
        }
        ...
        for(int i = 0; i < collageDimension; i++) {
            for(int j = 0; j < collageDimension; j++) {
                if(i == collageRow && j == collageCol) {
                    for(int m = 0; m < tileDimension; m++) {
                        for(int n = 0; n < tileDimension; n++) {
                            Color color = collagePicture.get(m +(tileDimension*j), n + (tileDimension*i));
                            ...
                            if(component.equals("red")) {
                                collagePicture.set(m +(tileDimension*j), n + (tileDimension*i), new Color(r, 0, 0));
                            }
                            else if(component.equals("green")) {
                                collagePicture.set(m +(tileDimension*j), n + (tileDimension*i), new Color(0, g, 0));
                            }
                            else if(component.equals("blue")) {
                                collagePicture.set(m +(tileDimension*j), n + (tileDimension*i), new Color(0, 0, b));
                            }
                        }
                    }
                }
    ...
    public void replaceTile (String filename,  int collageCol, int collageRow) {
        ...
        Picture scaleReplace = new Picture(tileDimension, tileDimension);

        scale(replace,scaleReplace);
        ...
        for(int i = 0; i < collageDimension; i++) {
            for(int j = 0; j < collageDimension; j++) {
                if(i == collageRow && j == collageCol) {
                    for(int m = 0; m < tileDimension; m++) {
                        for(int n = 0; n < tileDimension; n++) {
                            Color color = scaleReplace.get(m, n);
                            collagePicture.set(m+(j*tileDimension), n+(i*tileDimension), color);
                        }
                    }
                    break; 
                }
            ...
            for(int j = 0; j < collageDimension; j++) {
               if(i == collageRow && j == collageCol) {
                for(int m = 0; m < tileDimension; m++) {
                    for(int n = 0; n < tileDimension; n++) {
                        Color color = collagePicture.get(m + (tileDimension * j), n + (tileDimension * i));
                        Color colorGray = toGray(color);
                        collagePicture.set(m + (tileDimension * j), n + (tileDimension * i), colorGray);
                    }
                 }
               }
            }
